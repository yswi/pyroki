...
def normalize_with_norm(
    ...
    result_norm = norm[..., 0]  # Remove last dim for norm
...
def closest_segment_to_segment_points(
    ...
    s = jnp.where(denom < _EPS, s_parallel, s_num / (denom + _EPS))
    t = jnp.where(denom < _EPS, t_parallel, t_num / (denom + _EPS))
    ...
    t_final = jnp.where(
        jnp.abs(s - s_clamped) > _EPS, jnp.clip(t_recomp, 0.0, 1.0), t_clamped
    )
    ...
    s_final = jnp.where(
        jnp.abs(t - t_final) > _EPS, jnp.clip(s_recomp, 0.0, 1.0), s_clamped
    )
