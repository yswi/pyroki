...
from ._geometry import CollGeom, Capsule
...
class RobotCollision:
    ...
    def from_urdf(
        ...
        assert capsules.get_batch_axes() == (link_info.count,)
        ...
        logger.info(
            f"Created RobotCollision with {link_info.count} links and "
            f"{len(active_idx_i)} active self-collision pairs."
        )

        return RobotCollision(
            num_links=link_info.count,
            link_names=link_name_list,
...
def compute_self_collision_distance(
    ...
    Returns:
        Signed distances for each active pair (defined by robot_coll.active_idx_i/j)
        for each batch element. Shape: (*batch, num_active_pairs).
    ...
    assert coll_world.get_batch_axes() == (*batch_axes, robot_coll.num_links)
    ...
    active_distances = dist_matrix[
        ..., robot_coll.active_idx_i, robot_coll.active_idx_j
    ]

    # Expected shape check
    num_active_pairs = len(robot_coll.active_idx_i)
    assert active_distances.shape == (*batch_axes, num_active_pairs)

    return active_distances


# Define the vmap function specific for N links vs 1 world geom
_collide_links_vs_world = jax.vmap(collide, in_axes=(-2, None))
...
def compute_world_collision_distance(
    ...
    # Shape: (*batch_cfg, N, ...)
    coll_robot_world = robot_coll.at_config(robot, cfg)
    N = robot_coll.num_links
